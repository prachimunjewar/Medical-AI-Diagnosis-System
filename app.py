...
.stSuccess { background: #0f2d1a !important; border-color: #10B981 !important; }
.stWarning { background: #1a1200 !important; border-color: #F59E0B !important; }
.stInfo { background: #0d1b3e !important; border-color: #3B82F6 !important; }
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["📊  Data Analysis", "🤖  Train Models", "🔬  Patient Diagnosis", "🧠  Brain Tumor MRI"],
...
if "Data Analysis" in page:
    ...
    <div class="stat-grid">
        <div class="stat-card">
            ...
            <div class="stat-sub">Patient entries</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Features</div>
            <div class="stat-value">{stats['features']}</div>
            <div class="stat-sub">Clinical variables</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Diabetic</div>
            <div class="stat-value" style='color:#EF4444;'>{stats['diabetic']}</div>
            <div class="stat-sub">{round(stats['diabetic']/stats['total_records']*100,1)}% of total</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Non-Diabetic</div>
            <div class="stat-value" style='color:#10B981;'>{stats['non_diabetic']}</div>
            <div class="stat-sub">{round(stats['non_diabetic']/stats['total_records']*100,1)}% of total</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# ─── PAGE 2: TRAIN ───────────────────────────────────────────────────────────
    ...
    <div class="info-card">
        ...
        <ol class="step-list">
            <li class="step-item"><div><strong>SMOTE</strong> — Synthetic oversampling to balance diabetic vs non-diabetic classes</div></li>
            <li class="step-item"><div><strong>Sklearn Pipeline</strong> — StandardScaler + Classifier in one pipeline</div></li>
            <li class="step-item"><div><strong>GridSearchCV</strong> — 5-fold cross validation to find best hyperparameters</div></li>
    ...
    if "ml_results" in st.session_state:
        ...
        with col1:
            st.markdown("""
            <div class="info-card">
                <div class="info-card-title">🌲 Random Forest</div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown(f"""
        ...
        with col2:
            st.markdown("""
            <div class="info-card">
                <div class="info-card-title">⚡ XGBoost</div>
            </div>
            """, unsafe_allow_html=True)
            st.markdown(f"""
            <div class="metric-row">
...
# ─── PAGE 3: PATIENT DIAGNOSIS ────────────────────────────────────────────────
    ...
    if tumor_result:
        ...
        <div class="tumor-card">
            ...
            <div style='font-size:0.82rem; color:#94A3B8; margin-top:4px;'>This will be included in the medical report below</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if st.button("🔬 Run Diagnosis & Generate AI Report"):
        with st.spinner("Analysing patient data and generating report..."):
        ...
        <div class="disclaimer">
            ⚠️ This report is AI-generated for educational purposes only and is not a substitute for professional medical advice, diagnosis, or treatment.
        </div>
        """, unsafe_allow_html=True)
...
# ─── PAGE 4: BRAIN TUMOR MRI ─────────────────────────────────────────────────
    ...
    <div class="hero-banner">
        ...
        <p class="hero-sub">Upload a brain MRI scan for AI-powered classification using ResNet50 Transfer Learning</p>
        <div class="hero-tags">
            <span class="hero-tag">ResNet50</span>
            <span class="hero-tag">Transfer Learning</span>
            <span class="hero-tag">4-Class Classification</span>
            <span class="hero-tag">Glioma · Meningioma · Pituitary · No Tumor</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not cnn_model:
        st.markdown("""
        <div class="info-card">
            <div class="info-card-title">🔧 CNN Model Not Found</div>
            <p style='color:#64748B; font-size:0.9rem; margin-bottom:1rem;'>Train the brain tumor CNN first by following these steps:</p>
            <ol class="step-list">
                ...
                <li class="step-item"><div>Extract to <code style='background:#1E2D4A; padding:2px 6px; border-radius:4px;'>data/brain_tumor/Training/</code> with 4 class folders inside</div></li>
                <li class="step-item"><div>Run <code style='background:#1E2D4A; padding:2px 6px; border-radius:4px;'>python train_cnn.py</code> in terminal (~15-20 mins)</div></li>
                <li class="step-item"><div>Refresh this page — the upload section will appear automatically</div></li>
            </ol>
        </div>
        """, unsafe_allow_html=True)
        st.stop()
    ...
    if uploaded:
        ...
        st.markdown("""
        <div class="disclaimer">
            🧠 MRI result saved to session. Go to <strong>Patient Diagnosis</strong> page to include this finding in the full AI medical report.
        </div>
        """, unsafe_allow_html=True)
