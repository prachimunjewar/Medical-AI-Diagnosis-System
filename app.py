...
section[data-testid="stSidebar"] { background: linear-gradient(180deg, #0D1530 0%, #0A0F1E 100%); border-right: 1px solid #1E2D4A; }
section[data-testid="stSidebar"] * { color: #CBD5E1 !important; }
section[data-testid="stSidebar"] .stRadio label { background: #111827; border: 1px solid #1E2D4A; border-radius: 10px; padding: 10px 16px; margin-bottom: 6px; display: block; transition: all 0.2s; cursor: pointer; }
section[data-testid="stSidebar"] .stRadio label:hover { border-color: #3B82F6; background: #1a2540; }
.main .block-container { background-color: #0A0F1E; padding: 2rem 3rem; max-width: 1200px; }
.hero-banner { background: linear-gradient(135deg, #0D1B3E 0%, #1a2a5e 50%, #0D1B3E 100%); border: 1px solid #1E3A6E; border-radius: 20px; padding: 2.5rem 3rem; margin-bottom: 2rem; position: relative; overflow: hidden; }
.hero-title { font-family: 'Space Grotesk', sans-serif; font-size: 2.4rem; font-weight: 700; background: linear-gradient(135deg, #60A5FA, #A78BFA, #34D399); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin: 0 0 0.5rem; }
.hero-sub { color: #94A3B8; font-size: 1rem; margin: 0; }
.hero-tags { display: flex; gap: 10px; margin-top: 1.2rem; flex-wrap: wrap; }
.hero-tag { background: rgba(59,130,246,0.15); border: 1px solid rgba(59,130,246,0.3); color: #93C5FD; padding: 4px 14px; border-radius: 20px; font-size: 0.78rem; font-weight: 500; }
.section-header { font-family: 'Space Grotesk', sans-serif; font-size: 1.5rem; font-weight: 600; color: #F1F5F9; margin: 1.5rem 0 1rem; padding-bottom: 0.5rem; border-bottom: 1px solid #1E2D4A; }
.stat-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; margin-bottom: 1.5rem; }
.stat-card { background: linear-gradient(135deg, #111827, #1a2235); border: 1px solid #1E2D4A; border-radius: 14px; padding: 1.2rem 1.5rem; transition: border-color 0.2s; }
...
.stButton > button { background: linear-gradient(135deg, #3B82F6, #8B5CF6) !important; color: white !important; border: none !important; border-radius: 10px !important; padding: 0.6rem 2rem !important; font-weight: 600 !important; font-size: 0.95rem !important; transition: opacity 0.2s !important; width: 100%; }
.stButton > button:hover { opacity: 0.88 !important; }
.report-box { background: #0D1530; border: 1px solid #1E3A6E; border-radius: 14px; padding: 2rem; line-height: 1.8; color: #CBD5E1; font-size: 0.95rem; }
.tumor-card { background: linear-gradient(135deg, #1a0f2e, #2d1a4a); border: 2px solid #8B5CF6; border-radius: 16px; padding: 1.5rem 2rem; margin: 1rem 0; }
.disclaimer { background: #111827; border-left: 3px solid #F59E0B; border-radius: 0 10px 10px 0; padding: 0.8rem 1.2rem; font-size: 0.82rem; color: #94A3B8; margin-top: 1rem; }
.sql-badge { background: rgba(16,185,129,0.15); border: 1px solid rgba(16,185,129,0.3); color: #6EE7B7; padding: 2px 10px; border-radius: 20px; font-size: 0.72rem; font-weight: 600; font-family: monospace; margin-left: 8px; }
</style>
...
# ─── PAGE 2: TRAIN ───────────────────────────────────────────────────────────
    ...
    <div class="hero-banner">
        ...
        <p class="hero-sub">Train XGBoost and Random Forest with SMOTE oversampling and GridSearchCV hyperparameter tuning</p>
        <div class="hero-tags">
            <span class="hero-tag">XGBoost</span>
            <span class="hero-tag">Random Forest</span>
            <span class="hero-tag">SMOTE</span>
            <span class="hero-tag">GridSearchCV</span>
            <span class="hero-tag">Cross Validation</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# ─── PAGE 3: PATIENT DIAGNOSIS ────────────────────────────────────────────────
    ...
    <div class="hero-banner">
        ...
        <div class="hero-tags">
            <span class="hero-tag">Risk Prediction</span>
            <span class="hero-tag">Groq LLaMA 3.3</span>
            <span class="hero-tag">Medical Report</span>
            <span class="hero-tag">SQLite Storage</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# ─── PAGE 4: BRAIN TUMOR MRI ─────────────────────────────────────────────────
    ...
    <div class="hero-banner">
        ...
        <div class="hero-tags">
            ...
            <span class="hero-tag">Glioma</span>
            <span class="hero-tag">Meningioma</span>
            <span class="hero-tag">Pituitary</span>
            <span class="hero-tag">No Tumor</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not cnn_model:
        ...
        <div class="info-card">
            ...
            <p style='color:#64748B; font-size:0.9rem; margin-bottom:1rem;'>Follow these steps to train the brain tumor classifier:</p>
            <ol style='color:#94A3B8; font-size:0.9rem; line-height:2;'>
                <li>Download Brain Tumor MRI Dataset from <a href='https://www.kaggle.com/datasets/masoudnickparvar/brain-tumor-mri-dataset' target='_blank' style='color:#60A5FA;'>Kaggle</a></li>
                <li>Extract to <code style='background:#1E2D4A; padding:2px 6px; border-radius:4px;'>data/brain_tumor/Training/</code> with 4 class folders</li>
                <li>Run <code style='background:#1E2D4A; padding:2px 6px; border-radius:4px;'>python train_cnn.py</code> in terminal</li>
                <li>Refresh this page after training completes</li>
            </ol>
        </div>
        """, unsafe_allow_html=True)
        st.stop()
    ...
    if uploaded:
        ...
        with col2:
            ...
            label_color = "#FCA5A5" if result["is_tumor"] else "#6EE7B7"

            st.markdown(f"""
            <div class="{border_class}">
                <div class="result-title" style='color:{color};'>{icon} {result['label']}</div>
                <p class="result-sub" style='color:{label_color};'>Confidence: <strong>{result['confidence']}%</strong></p>
...
# ─── PAGE 5: PATIENT RECORDS ─────────────────────────────────────────────────
    ...
    <div class="hero-banner">
        ...
        <p class="hero-sub">All diagnosis records stored in SQLite — filter, query and analyse historical patient data</p>
        <div class="hero-tags">
            <span class="hero-tag">SQLite</span>
            <span class="hero-tag">SELECT · WHERE · BETWEEN</span>
            <span class="hero-tag">COUNT · AVG</span>
            <span class="hero-tag">INSERT · DELETE</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="stat-grid">
        ...
        <div class="stat-card"><div class="stat-label">Diabetic</div><div class="stat-value" style='color:#EF4444;'>{stats['diabetic']}</div><div class="stat-sub">{round(stats['diabetic']/stats['total']*100,1)}% of total</div></div>
        <div class="stat-card"><div class="stat-label">Non-Diabetic</div><div class="stat-value" style='color:#10B981;'>{stats['non_diabetic']}</div><div class="stat-sub">{round(stats['non_diabetic']/stats['total']*100,1)}% of total</div></div>
        <div class="stat-card"><div class="stat-label">MRI Scans</div><div class="stat-value" style='color:#A78BFA;'>{stats['mri_scans']}</div><div class="stat-sub">Brain analyses</div></div>
    </div>
    <div class="stat-grid">
    ...
    st.markdown(f"<p style='color:#64748B; font-size:0.85rem;'>Showing <strong style='color:#60A5FA;'>{len(df)}</strong> records</p>", unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        st.dataframe(df[display_cols].rename(columns={
            "id": "ID", "timestamp": "Time", "age": "Age", "glucose": "Glucose",
            "bmi": "BMI", "prediction": "Outcome", "risk_probability": "Risk %",
        ...
        with col2:
            ...
            fig.update_layout(
                ...
                font_color="#CBD5E1", yaxis=dict(gridcolor="#1E2D4A"), xaxis=dict(gridcolor="#1E2D4A"),
            )
            st.plotly_chart(fig, use_container_width=True)
